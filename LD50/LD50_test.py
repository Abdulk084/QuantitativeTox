...
print(np.shape(x_test_ext_gc))
print(np.shape(A_test_ext_gc))
...
def convert_to_vectors(file_name):
    data = pd.read_csv(file_name)
    dataY_train = data[["ACTIVITY-LC50DM"	,"ACTIVITY-LC50"	,"ACTIVITY-IGC50" ,"ACTIVITY-LD50"]]
    ...
    X_smiles_array = np.asarray(data.smiles)
...
X_test_ext_sv,y_test_ext_sv= convert_to_vectors("external_test.csv")
...
y_test_ext_LC50DM_sv=y_test_ext_sv[:,0]
y_test_ext_LC50_sv=y_test_ext_sv[:,1]
y_test_ext_IGC50_sv=y_test_ext_sv[:,2]
y_test_ext_LD50_sv=y_test_ext_sv[:,3]
...
def convert_to_fv(file_name):
    bit_size = 1024

    Max_len = 93

    trfile = open(file_name, 'r')
    line = trfile.readline()
    dataX = []
    dataY = []
    for i, line in enumerate(trfile):
...
X_test_ext_fv,y_test_ext_fv= convert_to_fv("external_test.csv")
...
y_test_ext_LC50DM_fv=y_test_ext_fv[:,0]
y_test_ext_LC50_fv=y_test_ext_fv[:,1]
y_test_ext_IGC50_fv=y_test_ext_fv[:,2]
y_test_ext_LD50_fv=y_test_ext_fv[:,3]
...
model_sv_loaded = create_model_sv()
model_sv_loaded.load_weights("training_sv/cp_sv.ckpt")

model_fv_loaded = create_model_fv()
model_fv_loaded.load_weights("training_fv/cp_fv.ckpt")


# In[164]:


members = [model_fp_loaded, model_graph_loaded, model_desc_loaded, model_sv_loaded, model_fv_loaded]
...
def define_stacked_model(members):
    ...
    out0=[ensemble_outputs[0][0],ensemble_outputs[1][0],ensemble_outputs[2][0],ensemble_outputs[3][0]
         ,ensemble_outputs[4][0]]
    out1=[ensemble_outputs[0][1],ensemble_outputs[1][1],ensemble_outputs[2][1],ensemble_outputs[3][1]
         ,ensemble_outputs[4][1]]
    out2=[ensemble_outputs[0][2],ensemble_outputs[1][2],ensemble_outputs[2][2],ensemble_outputs[3][2]
         ,ensemble_outputs[4][2]]                 
    out3=[ensemble_outputs[0][3],ensemble_outputs[1][3],ensemble_outputs[2][3],ensemble_outputs[3][3]
         ,ensemble_outputs[4][3]]
...
pred_test_ext_stack_load =  model_stack .predict([X_test_ext_fp, [x_test_ext_gc, A_test_ext_gc],
                                       ...
                                       X_test_ext_desc_norm, X_test_ext_sv , X_test_ext_fv ])
...
pred_test_ext_stack_load = np.squeeze(pred_test_ext_stack_load[3][:1475])


test_ext_LD50_meta_r2 = np.array([test_ext_LD50_meta_r2])


test_ext_LD50_meta_mae = np.array([test_ext_LD50_meta_mae])


test_ext_LD50_meta_rmse = np.array([test_ext_LD50_meta_rmse])
...
pd.concat([
    ...
    pd.DataFrame(pred_test_ext_stack_load, columns=['pred_test_ext_stack_load_LD50']),

    pd.DataFrame(test_ext_LD50_meta_r2, columns=['test_ext_LD50_meta_r2']),

    pd.DataFrame(test_ext_LD50_meta_mae, columns=['test_ext_LD50_meta_mae']),

    pd.DataFrame(test_ext_LD50_meta_rmse, columns=['test_ext_LD50_meta_rmse'])


], axis=1).to_csv("LD50_test_results.csv", index=False)
